...
function App() {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-[#eee] text-black">
      <div
      ...
      >
        ...
        <h1 className="text-3xl font-bold mb-4">🌌 AstroBirthPic</h1>
        ...
        {apod ? (
          <div className="mt-6 text-center max-w-2xl">
            <h2 className="text-xl font-bold">{apod.title}</h2>
            <p className="text-gray-400 mt-2">{apod.explanation}</p>
            {apod.media_type === "image" ? (
              <img
                src={apod.url}
                alt={apod.title}
                className="mt-4 rounded-lg shadow-lg w-full transition-transform transform hover:scale-105 animate-fadeIn"
              />
            ) : (
              <iframe
                src={apod.url}
